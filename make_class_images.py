...
def batch_make(load_dir: str, save_dir: str, images_per_prompt: int = 4, replacement: Tuple[str, str] = ("", ""), 
               batch_size = 4, guidance_scale=7, num_inference_steps=25):
    ...
    for file_path in tqdm(prompt_files):
        ...
        while len(images) < images_per_prompt:
            ...
            images += pipe(prompt, guidance_scale=guidance_scale, negative_prompt=negative_prompt, 
                           num_images_per_prompt=n_images, num_inference_steps=num_inference_steps).images
